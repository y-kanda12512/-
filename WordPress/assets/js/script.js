...
// カテゴリ別のリンクを定義
var categoryLinks = {
  new: "/category/new", // 新着情報のリンク
  news: "/category/news", // ニュースのリンク
  tips: "/category/tips", // TIPSのリンク
  interview: "/category/interview", // インタビューのリンク
  default: "/home", // デフォルトのリンク
};
...
function changeTab() {
  ...
  // 選択されたタブのdata-category属性を取得
  var selectedCategory = this.getAttribute("data-category");
  console.log("Selected Category: ", selectedCategory); // ここでログを確認

  if (!selectedCategory) {
    selectedCategory = "all"; // デフォルトのカテゴリを設定
  }

  // カテゴリに応じたリンク先を設定
  var archiveLink = document.querySelector(".u-new-articles__archive-link-all-article");
  if (archiveLink) {
    archiveLink.setAttribute("href", categoryLinks[selectedCategory] || categoryLinks["default"]);
  }
...
tabs[0].classList.add("is-checked");
changeTab.call(tabs[0]); // 最初のタブを選択し、タブの切り替え処理を実行
